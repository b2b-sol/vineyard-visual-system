...
const visualDirectory = path.join(ROOT, "product-structure", "visuals");
const outputDirectory = await mkdtemp(
  path.join(os.tmpdir(), "vineyard-product-mermaid-"),
);
...
try {
  ...
  if (diagrams.length !== 5)
    throw new Error(
      `Expected 5 product Mermaid diagrams, received ${diagrams.length}`,
    );
  ...
  if (rendered.length !== diagrams.length)
    throw new Error(
      `Expected ${diagrams.length} product SVGs, received ${rendered.length}`,
    );
  console.log(
    `✓ rendered ${diagrams.length} product Mermaid diagrams without syntax errors`,
  );
} finally {
